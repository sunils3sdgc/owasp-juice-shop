...
describe('/#/contact', () => {
    ...
    describe('challenge "forgedFeedback"', () => {
        ...
        it('should be possible to provide feedback as another user', () => {
            ...
            cy.get('.customer-table > .mat-table > :nth-child(8) > .cdk-column-user').then(($val) => {
                if ($val.text() !== ' 2') {
                    cy.get('.customer-table > .mat-table > :nth-child(9) > .cdk-column-user').should('contain.text', '2');
                }
                else {
        ...
        xit('should be possible to trick the sanitization with a masked XSS attack', () => {
            cy.task('disableOnContainerEnv').then((disableOnContainerEnv) => {
                if (!disableOnContainerEnv) {
                    cy.get('#comment').type('<<script>Foo</script>iframe src="javascript:alert(`xss`)">');
                    cy.get('#rating').click();
                    cy.get('#submitButton').click();
                    cy.visit('/#/about');
    ...
    describe('challenge "weirdCrypto"', () => {
        it('should be possible to post weird crypto algorithm/library as feedback', () => {
            cy.get('#comment').type('The following libraries are bad for crypto: z85, base85, md5 and hashids');
            cy.get('#rating').click();
            cy.get('#submitButton').click();
    ...
    describe('challenge "typosquattingNpm"', () => {
        it('should be possible to post typosquatting NPM package as feedback', () => {
            cy.get('#comment').type('You are a typosquatting victim of this NPM package: epilogue-js');
            cy.get('#rating').click();
            cy.get('#submitButton').click();
    ...
    describe('challenge "typosquattingAngular"', () => {
        it('should be possible to post typosquatting Bower package as feedback', () => {
            cy.get('#comment').type('You are a typosquatting victim of this Bower package: anuglar2-qrcode');
            cy.get('#rating').click();
            cy.get('#submitButton').click();
    ...
    describe('challenge "hiddenImage"', () => {
        it('should be possible to post hidden character name as feedback', () => {
            cy.get('#comment').type('Pickle Rick is hiding behind one of the support team ladies');
            cy.get('#rating').click();
            cy.get('#submitButton').click();
    ...
    describe('challenge "zeroStars"', () => {
        it('should be possible to post feedback with zero stars by double-clicking rating widget', () => {
            ...
            cy.window().then(async () => {
                const response = await fetch(`${Cypress.env('baseUrl')}/rest/captcha/`, {
                    method: 'GET',
                    cache: 'no-cache',
                    headers: {
                ...
                async function sendPostRequest(captcha) {
                    const response = await fetch(`${Cypress.env('baseUrl')}/api/Feedbacks`, {
                        method: 'POST',
                        cache: 'no-cache',
                        headers: {
    ...
    describe('challenge "supplyChainAttack"', () => {
        it('should be possible to post GitHub issue URL reporting malicious eslint-scope package as feedback', () => {
            cy.get('#comment').type('Turn on 2FA! Now!!! https://github.com/eslint/eslint-scope/issues/39');
            cy.get('#rating').click();
            cy.get('#submitButton').click();
    ...
    describe('challenge "dlpPastebinDataLeak"', () => {
        it('should be possible to post dangerous ingredients of unsafe product as feedback', () => {
            cy.task('GetPastebinLeakProduct').then((products) => {
                cy.get('#comment').type(products.keywordsForPastebinDataLeakChallenge.toString());
            });
            cy.get('#rating').click();
            cy.get('#submitButton').click();
