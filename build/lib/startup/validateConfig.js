...
const validateConfig = ({ products = config.get('products'), memories = config.get('memories'), exitOnFailure = true } = {}) => {
    ...
    if (success) {
        logger.info(`Configuration ${colors.bold((_a = process.env.NODE_ENV) !== null && _a !== void 0 ? _a : 'default')} validated (${colors.green('OK')})`);
    }
    else {
        logger.warn(`Configuration ${colors.bold((_b = process.env.NODE_ENV) !== null && _b !== void 0 ? _b : 'default')} validated (${colors.red('NOT OK')})`);
