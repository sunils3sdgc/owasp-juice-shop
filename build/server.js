...
while (!expectedModels.every(model => Object.keys(models_1.sequelize.models).includes(model))) {
    logger.info(`Entity models ${colors.bold(Object.keys(models_1.sequelize.models).length)} of ${colors.bold(expectedModels.length)} are initialized (${colors.yellow('WAITING')})`);
}
logger.info(`Entity models ${colors.bold(Object.keys(models_1.sequelize.models).length)} of ${colors.bold(expectedModels.length)} are initialized (${colors.green('OK')})`);
...
async function start(readyCallback) {
    ...
    await datacreator();
    datacreatorEnd();
    const port = (_a = process.env.PORT) !== null && _a !== void 0 ? _a : config.get('server.port');
    process.env.BASE_PATH = (_b = process.env.BASE_PATH) !== null && _b !== void 0 ? _b : config.get('server.basePath');
